...
class VectorKernel
{
  ...
  public:
    VectorKernel(double x = 0, double y = 0, double z = 0) : x_(x), y_(y), z_(z), length_(-1) {}
...
class Vector
{
  ...
  public:
    Vector(double x = 0, double y = 0, double z = 0) : Kernel(x, y, z) {}
    ...
    {
        return std::sqrt(Kernel.getX() * Kernel.getX() + Kernel.getY() * Kernel.getY() +
...
{
    double x1 = Kernel.getX(), x2 = vec.Kernel.getX(), y1 = Kernel.getY(), y2 = vec.Kernel.getY(),
